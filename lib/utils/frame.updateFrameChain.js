...
const createOrUpdate = require('./frame.createOrUpdate');
  ...
  return getCurrentPlayer(gameId)
    .then((player) => {
      ...
      if (playerDoc) {
        ...
        return updateScore(playerDoc, updateBody, flag);
      }
    })
    .then(() => {
      if (playerDoc) {
    ...
    .then((round) => {
      ...
      if (currentRound !== 0) {
        ...
        throw new Error('Game is completed and can no longer be updated', gameId);
      }
    })
    .then((playersList) => {
      if (playersList) {
    ...
    .catch((err) => {
      ...
      throw new Error('Trouble updating current frame',err);
    })
}
