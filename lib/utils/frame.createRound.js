...
/**
 * @Description
 *    All in one function to create a round for a given list of player
 * @param players - required
 * @param frameNumber - optional, defaults to 1
 * @param gameNumber - optional, defaults to created ID for first round
 */
function createRound(players, frameNumber, gameNumber) {
  ...
  return Promise.all(promises)
    ...
    .catch((err) => {
      log.error("Error creating round", err.message);
      log.debug('Stack', err.stack);
      return Promise.reject(Error("Error creating round " + frameNumber + " for players", players, ": ", err.message));
    })
}
